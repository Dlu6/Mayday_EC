...
const CallHistory = ({ open, onClose, onCallNumber }) => {
  ...
  // Setup realtime socket updates for call history
  useEffect(() => {
    // Fetch initial call history
    fetchCallHistory();

    // Set up realtime updates via socket - this is the primary update mechanism
    const socket = callHistoryService.setupRealtimeUpdates((newRecord) => {
      if (newRecord) {
        console.log("Received realtime call history update:", newRecord);
        // Update or add the record in the calls list
        setCalls((prevCalls) => {
          // Check if this record already exists
          const existingIndex = prevCalls.findIndex(
            (call) => call.id === newRecord.id
          );
          
          if (existingIndex >= 0) {
            // Update existing record with new data
            const updatedCalls = [...prevCalls];
            updatedCalls[existingIndex] = newRecord;
            return updatedCalls;
          } else {
            // Add new record at the beginning
            return [newRecord, ...prevCalls].slice(0, 50);
          }
        });
        
        // Update last timestamp
        if (newRecord.timestamp) {
          setLastTimestamp(newRecord.timestamp);
        }
      }
    });

    // Set up fallback polling interval (less frequent since we have realtime updates)
    const interval = setInterval(() => {
      if (open) {
        // Fallback poll every 60 seconds in case socket misses something
        fetchCallHistory(false, true);
      }
    }, 60000); // Reduced to 60 seconds as fallback only

    setPollingInterval(interval);

    // Clean up interval and socket on unmount
    return () => {
      if (interval) {
