...
export const useCallState = (sipService, sipCallService) => {
  ...
  const safePlayAudio = useCallback((audioSrc, volume = 0.5) => {
    ...
    audio.volume = volume;
    audio.loop = true;
    audio.muted = false; // ensure not muted before attempting playback

    // Route ringtone to default/preferred output if supported
    try {
      if (typeof audio.setSinkId === "function") {
        const preferredOutput =
          localStorage.getItem("preferredOutputDeviceId") || "default";
        if (preferredOutput) {
          audio
            .setSinkId(preferredOutput)
            .catch((e) =>
              console.warn("setSinkId failed for ringtone:", e?.message || e)
            );
        }
      }
    } catch (_) { }
