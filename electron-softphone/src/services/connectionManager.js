...
async function reconnectSip() {
  try {
    ...
    // CRITICAL: Skip if SIP is already initializing to avoid race conditions
    if (sipService?.state?.isInitializing) {
      console.log("⏳ SIP service is initializing, skipping reconnection (will complete on its own)");
      return true; // Return true to indicate it's being handled
    }

    // CRITICAL: Skip if SIP is already connected
    if (sipService?.state?.isConnected && sipService?.state?.userAgent) {
      console.log("✅ SIP service already connected, skipping reconnection");
      return true;
    }
    ...
    if (
      ...
      console.warn("⚠️ SIP service not available for reconnection (no config or service)");
      return false;
    }
  } catch (error) {
...
function startHealthMonitoring() {
  ...
  healthState.healthCheckTimer = setInterval(() => {
    try {
      // CRITICAL: Skip health checks during authentication or when no token
      if (window.isLoggingOut || window.isAuthenticating) {
        return;
      }
      // CRITICAL: Stop health checks if no auth token (session expired)
      const token = localStorage.getItem("authToken") || sessionStorage.getItem("authToken");
      if (!token) {
        // console.log("🔐 Health check skipped - no auth token");
        return;
      }
      performHealthCheck();
    } catch (error) {
  ...
  healthState.heartbeatTimer = setInterval(() => {
    try {
      // CRITICAL: Skip heartbeats during authentication or when no token
      if (window.isLoggingOut || window.isAuthenticating) {
        return;
      }
      // CRITICAL: Stop heartbeats if no auth token (session expired)
      const token = localStorage.getItem("authToken") || sessionStorage.getItem("authToken");
      if (!token) {
