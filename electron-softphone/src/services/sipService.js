...
async function reconnect() {
  ...
  // CRITICAL: Save config BEFORE disconnect clears it
  const savedConfig = state.lastConfig;
...
export const sipService = {
  initialize: async (config) => {
    ...
    try {
      ...
      // Update connected state after successful connection
      state.isConnected = true;
      console.log("✅ SIP service initialized successfully");
    } catch (error) {
      console.error("❌ SIP initialization failed:", error);
      // Reset state on failure - but keep lastConfig for potential retry
      state.isConnected = false;
      // Don't clear lastConfig here - keep it for reconnection attempts
      throw error;
    } finally {
      state.isInitializing = false;
    }
    ...
    try {
      ...
      if (state.consultationCall?.originalSession) {
        ...
        try {
          ...
          console.warn("Failed to restore audio after transfer failure:", audioError);
        }
      }
  ...
  cancelAttendedTransfer: async () => {
    try {
      ...
      if (originalSession) {
        ...
        // Restore audio to original call with retry logic
        try {
          const remoteStream =
          ...
          if (remoteStream && state.audioElement) {
            state.audioElement.srcObject = remoteStream;
            await state.audioElement.play();
            console.log("Audio restored to original call successfully");
          } else {
            console.warn("No remote stream available for audio restoration");
          }
        } catch (audioError) {
          console.warn("Initial audio restoration failed, retrying...", audioError);
          ...
          setTimeout(async () => {
            try {
              ...
              if (remoteStream && state.audioElement) {
                state.audioElement.srcObject = remoteStream;
                await state.audioElement.play();
                console.log("Audio restored on retry");
              }
            } catch (retryError) {
              console.error("Audio restoration retry failed:", retryError);
            }
          }, 200);
        }
      }
