    ...
    if (isDevelopmentLicense && license.status === "active") {
      ...
      switch (license.status) {
        case "active": {
          ...
          fourteenDaysFromNow.setDate(now.getDate() + 15);
    ...
    return (
      <>
        <Card title="License Details">
          <Alert
            title={`License Status: ${displayStatus.toUpperCase()}`}
        ...
        <Card title="Plan Features" style={{ marginTop: 24 }}>
          ...
          >
            {Object.entries(licensedFeatures).map(([featureKey, isEnabled]) => {
              const featureName = getFeatureDisplayName(featureKey);
              const icon = getFeatureIcon(featureKey);
              return (
                <Tooltip key={featureKey} title={featureName} placement="bottom">
                  <Card
                    hoverable
                    style={{
                      textAlign: "center",
