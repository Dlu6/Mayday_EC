...
import { updateTrunkDetailsAsync } from "../features/trunks/trunkSlice";
...
const TrunkEdit = () => {
  ...
  const handleInputChange = (event) => {
    ...
    // Check if the field should be an array (for multi-select fields)
    if (["nat", "transport", "codecs", "insecure"].includes(name)) {
...
const AdvancedTabContent = ({ formData, handleInputChange, disabled }) => {
  ...
  const sendRemotePartyIdHeader = [
    { value: "yes", label: "Yes" },
    { value: "no", label: "No" },
    { value: "P-Asserted-Identity", label: "P-Asserted-Identity" },
  ];
  const encryption = [
    { value: "yes", label: "Yes" },
    { value: "no", label: "No" },
