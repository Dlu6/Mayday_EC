...
export const exportReport = async (req, res) => {
  try {
    ...
    switch (reportType) {
      ...
      case "datatool":
        try {
          // MAJOR CHANGE: Directly call the controller function instead of making HTTP request
          // Create a mock req and res for the controller function
          const mockReq = { query: { startDate, endDate } };

          // Use a promise to get the data from the controller function
          const dataToolData = await new Promise((resolve, reject) => {
            // Create a mock response object that captures the JSON data
            const mockRes = {
              status: () => mockRes,
              json: (data) => resolve(data),
            };
            ...
            getDataToolMetrics(mockReq, mockRes).catch(reject);
          });

          if (!dataToolData || Object.keys(dataToolData).length === 0) {
            return res.status(404).json({
              message: "No data available for the selected date range",
            });
          }

          // Create CSV sections for different metrics
          const sections = [];
          ...
          sections.push([
            ["Data Tool Summary"],
            ["Metric", "Value"],
            ["Total Cases", dataToolData.totalCases],
            ["Total Sessions", dataToolData.totalSessions],
            ["Active Counselors", dataToolData.activeUsers],
            [""],
          ]);

          // Cases by Difficulty
          if (
            dataToolData.casesByDifficulty &&
            dataToolData.casesByDifficulty.length > 0
          ) {
            sections.push([
              ["Cases by Difficulty"],
              ["Difficulty", "Count", "Percentage"],
              ...dataToolData.casesByDifficulty.map((item) => [
                item.name,
                item.value,
                `${((item.value / dataToolData.totalCases) * 100).toFixed(2)}%`,
              ]),
              [""],
            ]);
          }
          ...
          // Cases by Region
          if (
            dataToolData.casesByRegion &&
            dataToolData.casesByRegion.length > 0
          ) {
            sections.push([
              ["Cases by Region"],
              ["Region", "Count", "Percentage"],
              ...dataToolData.casesByRegion.map((item) => [
                item.name,
                item.value,
                `${((item.value / dataToolData.totalCases) * 100).toFixed(2)}%`,
              ]),
          ...
          // Cases by Sex
          if (dataToolData.casesBySex && dataToolData.casesBySex.length > 0) {
            sections.push([
              ["Cases by Sex"],
              ["Sex", "Count", "Percentage"],
              ...dataToolData.casesBySex.map((item) => [
                item.name,
                item.value,
                `${((item.value / dataToolData.totalCases) * 100).toFixed(2)}%`,
              ]),
          ...
          // Cases by Age
          if (dataToolData.casesByAge && dataToolData.casesByAge.length > 0) {
            sections.push([
              ["Cases by Age Group"],
              ["Age Group", "Count", "Percentage"],
              ...dataToolData.casesByAge.map((item) => [
                item.name,
                item.value,
                `${((item.value / dataToolData.totalCases) * 100).toFixed(2)}%`,
              ]),
          ...
          // Cases by Source
          if (
            dataToolData.casesBySource &&
            dataToolData.casesBySource.length > 0
          ) {
            sections.push([
              ["Cases by Source"],
              ["Source", "Count", "Percentage"],
              ...dataToolData.casesBySource.map((item) => [
                item.name,
                item.value,
                `${((item.value / dataToolData.totalCases) * 100).toFixed(2)}%`,
              ]),
          ...
          if (
            dataToolData.counselorPerformance &&
            dataToolData.counselorPerformance.length > 0
          ) {
            sections.push([
              ["Counselor Performance"],
              ["Name", "Role", "Cases", "Sessions"],
              ...dataToolData.counselorPerformance.map((item) => [
                item.name,
                item.role,
                item.cases,
                item.sessions,
          ...
          // Flatten sections and convert to CSV
          const flattenedData = sections.flat();
          ...
          console.error("Error generating Data Tool report:", error);
          return res.status(500).json({
            message: "Failed to generate Data Tool report",
            error: error.message,
          });
        }

      case "datatool-all-time":
        try {
          // MAJOR CHANGE: Directly call the controller function instead of making HTTP request
          const dataToolData = await new Promise((resolve, reject) => {
            // Create a mock response object that captures the JSON data
            ...
            // Call the controller function directly
            getDataToolAllTimeMetrics({}, mockRes).catch(reject);
          });
          ...
          if (
            ...
            sections.push([
              ...
              ["Name", "Role", "Cases", "Sessions", "Cases %", "Sessions %"],
              ...dataToolData.counselorPerformance.map((item) => [
                item.name,
                item.role,
                item.cases,
                item.sessions,
                `${((item.cases / dataToolData.totalCases) * 100).toFixed(2)}%`,
                `${((item.sessions / dataToolData.totalSessions) * 100).toFixed(
                  2
                )}%`,
              ]),
              [""],
          ...
          // Get detailed records for all cases (directly using controller function)
          const allPosts = await new Promise((resolve, reject) => {
            const mockRes = {
              status: () => mockRes,
              json: (data) => resolve(data),
            };
            getAllPostsForReport({}, mockRes).catch(reject);
          });

          if (allPosts && allPosts.data && allPosts.data.length > 0) {
            // Fetch creator user info for name/role mapping
            const creatorIds = [
              ...new Set(
                allPosts.data
            ...
            const userMap = new Map(
              users.map((u) => [
                String(u._id),
                { name: u.name, role: u.user_role },
              ])
            );

            // Full-field export mirroring the DataTool schema
            const headers = [
              "caseId",
              "createdAt",
              "user_id",
            ...
            sections.push([["All Case Records"], headers]);
            sections.push(
              allPosts.data.map((post) => {
                const creatorInfo = userMap.get(String(post.creator)) || {};
                const row = {
                  caseId: post._id,
                  ...
                  sameAsCaller: post.sameAsCaller || "",
                  ...
                  name: post.name || "",
                  consentV1Text: post.consentV1Text || "",
                  consentV1Accepted: post.consentV1Accepted ?? "",
                  consentV1AcceptedAt: post.consentV1AcceptedAt
                    ? new Date(post.consentV1AcceptedAt).toISOString()
                    : "",
                  feedbackConsentText: post.feedbackConsentText || "",
                  feedbackConsentAccepted: post.feedbackConsentAccepted ?? "",
                  feedbackConsentAcceptedAt: post.feedbackConsentAcceptedAt
                    ? new Date(post.feedbackConsentAcceptedAt).toISOString()
                    : "",
                  sessionCount:
                    post.sessionList && post.sessionList.length
                      ? post.sessionList.length
                      : 1,
                  sessionList: post.sessionList || [],
                };
          ...
          filename = `datatool-all-time-report-${
            new Date().toISOString().split("T")[0]
          }.csv`;
          ...
          console.error("Error generating All-Time Data Tool report:", error);
          return res.status(500).json({
            message: "Failed to generate All-Time Data Tool report",
            error: error.message,
          });
        }
