    ...
    if (!isDev) {
      ...
      try {
        const { stdout } = await execAsync(
          'sudo /usr/sbin/asterisk -rx "core show version" 2>/dev/null || asterisk -rx "core show version" 2>/dev/null'
        );
        services.asterisk = stdout.includes("Asterisk");
      } catch (e) {
        // Fallback: Check if asterisk process is running
        try {
          const { stdout: psOutput } = await execAsync('pgrep -x asterisk || pgrep -f "/usr/sbin/asterisk"');
          services.asterisk = psOutput.trim().length > 0;
        } catch (psError) {
      ...
      // Check AMI status - verify manager is enabled and listening
      try {
        // Check if AMI manager is enabled and has settings configured
        const { stdout: managerStatus } = await execAsync(
          'sudo /usr/sbin/asterisk -rx "manager show settings" 2>/dev/null || asterisk -rx "manager show settings" 2>/dev/null'
        );
        
        // AMI is running if we can get settings and it shows enabled
        const isEnabled = managerStatus.includes("Manager (AMI)") || 
                         managerStatus.includes("enabled") ||
                         managerStatus.includes("HTTP Enabled") ||
                         managerStatus.toLowerCase().includes("yes");
        
        services.ami = isEnabled;
        
        if (!services.ami) {
          console.log("AMI Manager Status:", managerStatus);
        }
      } catch (e) {
        // Fallback: Try to connect to AMI port directly
        try {
          const net = await import('net');
          const amiPort = process.env.AMI_PORT || 5038;
          const amiHost = process.env.AMI_HOST || '127.0.0.1';
          
          services.ami = await new Promise((resolve) => {
          ...
          console.error("AMI socket check failed:", socketError);
          services.ami = false;
        }
      }

      // Check ARI status - try HTTP endpoint first
      try {
        const ariPort = process.env.ARI_PORT || 8088;
        const ariHost = process.env.ARI_HOST || '127.0.0.1';
        
        // Try to reach ARI HTTP endpoint
        const http = await import('http');
        services.ari = await new Promise((resolve) => {
