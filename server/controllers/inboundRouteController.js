              ...
              await VoiceExtension.create(
                {
                  ...
                  exten: phone_number, // from req.body
                ...
                await VoiceExtension.create(
                  {
                    ...
                    app: "Answer",
                    type: "inbound",
                    description: `Answer call for Queue ${queueName}`,
                    isApp: false,
                    intervalId,
                    interval: intervalId,
                    answer: true,
                ...
                await VoiceExtension.create(
                  {
                    ...
                    priority: currentPriority++,
                    app: "MixMonitor",
                    appdata: `${recordingPath},bWv(12,10),r(CALL_RECORDING_AUDIO_FORMAT=wav,CALL_RECORDING_QUALITY=wav49)`,
                    type: "inbound",
                ...
                await VoiceExtension.create(
                  {
                    ...
                    app: "Answer",
                    type: "inbound",
                    description: `Answer call for Queue ${queueName} (fallback)`,
                    isApp: false,
                    intervalId,
                    interval: intervalId,
                    answer: true,
...
const shouldAnswerCall = (appType) => {
  const autoAnswerApps = ["IVR", "Playback", "Queue"];
...
const parseAppSettings = (extension) => {
  switch (extension.app) {
    ...
    case "Queue": {
      const [queue, timeout, ...options] = extension.appdata.split(",");
      return {
        queue,
        timeout,
  ...
  try {
    ...
    await VoiceExtension.destroy({
      where: {
        ...
        exten: inboundRoute.phone_number,
      },
      transaction,
    });
          ...
          if (intervalObj) {
            try {
              ...
              if (app.type === "Queue") {
                ...
                // Ensure call is answered before recording starts
                await VoiceExtension.create(
                  {
                    context: inboundRoute.context,
                    exten: phone_number,
