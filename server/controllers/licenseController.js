...
export const endUserSession = async (req, res) => {
  try {
    ...
    if (!username || !feature) {
      return res.status(400).json({
        success: false,
        message: "Username and feature are required",
      });
    }

    // Find active session using Redis
    const sessionCheck = await redisSessionService.hasActiveSession(
      userId,
      feature
    );
    ...
    // Get current license for master notification
    const license = await licenseService.getCurrentLicense();

    // End session atomically in Redis
    ...
    if (license) {
      try {
        ...
        console.log(
          `📡 Notified master server of session termination for user ${username}`
        );
      } catch (masterError) {
        console.warn(
          "⚠️ Failed to notify master server of session termination:",
          masterError
        );
        // Don't fail the session termination if master notification fails
      }
    }

    console.log(
      `✅ Session ${sessionCheck.sessionId} ended successfully for user ${userId} (${username})`
    );
