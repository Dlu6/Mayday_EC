...
const UserModel = sequelize.define(
  "users",
  {
    ...
    allow: {
      type: DataTypes.STRING,
      allowNull: true,
      defaultValue: "ulaw;alaw;gsm", // Specifies default allowed codecs
    },
    insecure: {
      type: DataTypes.STRING,
      allowNull: true,
...
UserModel.hasOne(PJSIPAor, {
  foreignKey: "id", // Changed from user_id to id
  sourceKey: "extension", // Changed from id to extension
  as: "ps_aor",
  onDelete: "CASCADE",
});

// UserModel.hasOne(PJSIPEndpointIdentifier, {
//   foreignKey: "endpoint",
//   sourceKey: "extension",
//   as: "ps_endpoint_identifier", // Add an alias
//   onDelete: "CASCADE",
// });
...
UserModel.findExtensionByEmail = async function (email) {
  ...
  const user = await this.findOne({
    ...
    include: [
      {
        model: PJSIPEndpoint,
        as: "ps_endpoint", // Match the association alias
        required: true,
      },
      {
        model: PJSIPAuth,
        as: "ps_auth", // Match the association alias
        required: true,
      },
      {
        model: PJSIPAor,
        as: "ps_aor", // Match the association alias
        required: true,
      },
    ],
  });
