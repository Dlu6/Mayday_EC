    ...
    try {
      ...
      if (
        ...
        await FingerprintHistory.create({
          ...
          action_taken: "license_invalidated",
        });

        // Invalidate existing license
        ...
        console.log(
          "🚫 Existing license invalidated due to fingerprint change"
        );
        console.log("💡 Server will fallback to development license");
        console.log(
          "🔧 Generate new fingerprint and request license from administrator"
        );

        // Create development fallback license
        return {
          success: true,
...
export const startCacheCleanupService = () => {
  ...
  console.log("🧹 Cache and session cleanup service started");
};
...
// Session cleanup function to prevent session pollution
    ...
    const staleSessions = await ClientSession.findAll({
      where: {
        ...
        [Op.or]: [
          ...
          // Sessions without heartbeat in the last hour
          {
            [Op.or]: [
              { last_heartbeat: null },
              {
